...
from energyapp.consumption_profile.alpg import devices
from energyapp.consumption_profile.alpg import heatdemand
from energyapp.consumption_profile.alpg import persons
...
class Household:
    ...
    def generate(self):
        ...
        self.familyActivites = random.randint(external_inputs.familyOutingChanceMin,
                                              external_inputs.familyOutingChanceMax) / 100
    ...
    def scaleProfile(self):
        ...
        for k, v, in self.ConsumptionShare.items():
            ...
            multiplier = ((self.ConsumptionShare[k] / 100) * (
                    ((self.ConsumptionYearly / 365) * external_inputs.numDays) * 1000) * 60) / sumDevice  # joules
    ...
    def simulate(self):
        for day in range(external_inputs.startDay, external_inputs.numDays + external_inputs.startDay):
            dayOfWeek = day % 7
            ...
            # Smart devices
            if day - external_inputs.startDay < external_inputs.numDays - 1:
        ...
        if self.House.hasPV:
            ...
            self.PVProfile = self.Devices['PVPanel'].simulate(external_inputs.startDay,
                                                              external_inputs.numDays * ((3600 * 24) / 60),
                                                              self.House.pvArea, self.House.pvEfficiency,
                                                              self.House.pvAzimuth, self.House.pvElevation)
        else:
            self.PVProfile = [0] * external_inputs.numDays * int(24 * 3600 / 60)
        ...
        if (random.randint(1, 2) == 1):
            self.Fridges = [
                devices.DeviceFridge(
                    random.randint(external_inputs.ConsumptionFridgeBigMin, external_inputs.ConsumptionFridgeBigMax))]
        else:
            self.Fridges = [devices.DeviceFridge(
                ...
                devices.DeviceFridge(
                    random.randint(external_inputs.ConsumptionFridgeSmallMin,
                                   external_inputs.ConsumptionFridgeSmallMax))]
...
class HouseholdFamilyDualParent(Household):
    ...
    def __init__(self, parttime=False, jobless=False):
        ...
        self.ConsumptionYearly = profilegentools.gaussMinMax(
            external_inputs.yearlyConsumption + (700 * external_inputs.numKids), 500 + (
...
class HouseholdDualRetired(Household):
    def __init__(self):
        self.generate()
        self.ConsumptionYearly = profilegentools.gaussMinMax(external_inputs.yearlyConsumption,
                                                             600) * external_inputs.consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
