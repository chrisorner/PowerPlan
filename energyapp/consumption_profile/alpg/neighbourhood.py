...
from energyapp.consumption_profile.alpg import houses
from energyapp.consumption_profile.alpg.configs import external_inputs
from energyapp.consumption_profile.alpg.household_list import getHouseholdList
...
def neighbourhood():
    ...
    round(len(householdList) * (external_inputs.penetrationPV / 100))):
        ...
        if householdList[j].hasCHP == False and pvList[j] == 0:  # First supply houses without PV
            ...
            i = i + 1
    if (round(len(householdList) * (external_inputs.penetrationPV / 100))) > (round(len(householdList) * (
            external_inputs.penetrationCHP / 100))):  # If there are too much CHPs compared to PV, add some more CHPS
        while i < (round(len(householdList) * (external_inputs.penetrationCHP / 100))):
            j = random.randint(0, len(householdList) - 1)
    ...
    for i in range(0, len(drivingDistance)):
        if i < (round(len(householdList) * (external_inputs.penetrationEV / 100)) + round(
                len(householdList) * (external_inputs.penetrationPHEV / 100))):
    ...
    for i in range(0, len(householdList)):
        ...
        if pvList[i] == 1:
            ...
            area = round((householdList[
                              i].ConsumptionYearly / external_inputs.PVProductionPerYear) * 1.6)  # average panel is 1.6m2
        ...
        if batteryList[i] == 1:
            ...
            if householdList[i].hasEV:
                ...
                householdList[i].House.addBattery(external_inputs.capacityBatteryLarge,
                                                  external_inputs.powerBatteryLarge)  # Let's give it a nice battery!
            else:
                # NO EV, just some peak shaving:
                if (householdList[i].ConsumptionYearly > 2500):
                    householdList[i].House.addBattery(external_inputs.capacityBatteryMedium,
                                                      external_inputs.powerBatteryMedium)
                else:
                    householdList[i].House.addBattery(external_inputs.capacityBatterySmall,
                                                      external_inputs.powerBatterySmall)
