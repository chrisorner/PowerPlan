...
from energyapp.consumption_profile.alpg import writer
...
class DeviceLighting(Device):
    def simulate(self, timeintervals, occupancy, timestamp):
        sun = location.sun(date=datetime.date.fromtimestamp(timestamp), local=True)
        ...
        if CookingType == 10:
            ...
            for m in range(startCooking, startCooking + cookingDuration):
                if m < 10 + startCooking:
                    CookingProfile[m] += external_inputs.ConsumptionOven
            ...
            # CookingProfile[m] += ConsumptionStoveVentilation

            if (hasInductionCooking):
                inductionRatio = random.randint(3, 6)
                for m in range(startCooking, startCooking + cookingDuration):
                    if m < 6 + startCooking:
                        CookingProfile[m] += external_inputs.ConsumptionInductionStove
                    else:
                        CookingProfile[m] += round(external_inputs.ConsumptionInductionStove * (inductionRatio / 10))
            ...
            if random.random() < 0.2:
                ...
                for m in range(startCooking + randomOffset, startCooking + cookingDuration):
                    CookingProfile[m] += external_inputs.ConsumptionMicroWave
...
class DeviceVentilation(Device):
    ...
    def simulate(self, timeintervals, ventilation):
        ...
        for i in range(0, len(ventilation.VentilationProfile)):
            VentilationProfile.append(
                int((ventilation.VentilationProfile[i] / ventilation.MaxAirflow) * self.Consumption))
...
class DeviceSolarPanel(Device):
    def simulate(self, startday, timeintervals, pvArea, pvEfficiency, pvAzimuth, pvElevation):
        ...
        while (time < (startday * 24 * 60 * 60) + timeintervals * 60):
            modeltime = int(time - (time % external_inputs.weather_timebaseDataset) + int(
                external_inputs.weather_timebaseDataset / 2))

            d = datetime.datetime.utcfromtimestamp(1388534400 + modeltime)
            elevation = location.solar_elevation(d)
            azimuth = location.solar_azimuth(d)
            zenith = location.solar_zenith(d)

            index = int(math.floor(modeltime / external_inputs.weather_timebaseDataset))
            index = max(index, 0)
            try:
                irradiation = float(linecache.getline(external_inputs.weather_irradiation, index + 1))
            except:
                print(
                    "An error occurred reading the solar irradiation file. Make sure that the file is correct (e.g. only contains numbers), is long enough (make sure that it has a bit more data than the actual simulation. And, make sure that the file exists!")
                exit()

            GHI = (irradiation * 10000) / float(external_inputs.weather_timebaseDataset)
...
class DeviceWashingMachine(TimeShiftableDevice):
    ...
    def writeDevice(self, hnum):
        writer.writeDeviceTimeshiftable(self, hnum)
...
class DeviceElectricalVehicle(BufferTimeshiftableDevice):
    ...
    def writeDevice(self, hnum):
        writer.writeDeviceBufferTimeshiftable(self, hnum)
